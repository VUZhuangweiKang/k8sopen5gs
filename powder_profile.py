...
ueRANLink0 = request.Link("UERANLink1")
ueRANLink1 = request.Link("UERANLink2")

# Add node that will host the 5G Core Virtual Network Functions (AMF, SMF, UPF, etc).
for i in range(2):
    open5gs = request.RawPC("open5gs%d"%i)
...
for i in range(2):
    ...
    if i == 0:
        ueRANLink0.addNode(sim_ran)
    else:
        ueRANLink1.addNode(sim_ran)
...
for i in range(4):
    ...
    if i % 2 == 0:
        ueRANLink0.addNode(sim_ue)
    else:
        ueRANLink1.addNode(sim_ue)
